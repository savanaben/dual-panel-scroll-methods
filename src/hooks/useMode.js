...
export const useMode = () => {
  ...
  const availableModes = [
    ...
    { id: 'inner-frame', name: 'inner frame scrolls', description: 'An "inner frame" within the left column scrolls. A sticky position, vh-based height, and negative margins are used to fake a standard scrollable content panel. additional logic is needed to stop scroll propagation to the outer container when you scroll this inner frame.', disabled: true },
    { id: 'accordions', name: 'Accordions', description: 'Accordions sort of work, but it forces the students to hopefully make the layout better for themselves, and means changing how the sim tool and content displays somewhat significantly. Are we adding accordion support to set leaders? moving all set leader content to the sim tool?', disabled: true },
    { id: 'tabs', name: 'Tabs', description: 'The issue with tabs is how we sticky them to the top of the left panel, when the scrollable area is not the left panel (it\'s a higher container wrapping both panels using current DI scroll capability). If we use fancy logic like the Inner Frame method to stabalize the tabbed left panel, we might as well ditch tabs and just go with the Inner Frame method. Not stickying the tabs makes the ux worse, especially at zoom. Additionally, how are we authoring set leader text/img content (sub tabs in IBIS? all set leader content in the sim tool?).', disabled: true }
